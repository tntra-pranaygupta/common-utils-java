...
class FlywayMigrationTest {
    ...
    /**
     * Checks that V1 migration creates the outbox_event table.
     */
    @Test
    void v1ShouldCreateOutboxEventTableTest() {
        JdbcTemplate jdbc = new JdbcTemplate(dataSource);
        Integer count = jdbc.queryForObject(
                "SELECT COUNT(*) FROM information_schema.tables "
                        + "WHERE LOWER(table_name) = 'outbox_event'",
                Integer.class
        );
        assertThat(count).as("outbox_event table should exist").isEqualTo(1);
    }
    ...
    void v2FinancialTransactionMustHavePciCompliantColumnsTest() {
        ...
        List<String> requiredColumns = List.of(
                "id", "external_id", "pan_token", "pan_last_four",
